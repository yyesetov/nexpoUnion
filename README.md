...
| `GET`   | `/api/settings`       | Настройки сайта (`seasonClosed`)  |
...
| `/unpaid ID`          | Снять отметку оплаты                   |
| `/season`             | Открыть или закрыть сезон бронирования |

### Закрытие сезона

`/season` показывает текущее состояние и кнопку-переключатель. Когда сезон закрыт:

- сайт вместо формы показывает карточку «Сезон закрыт», календарь и поиск своих броней продолжают работать;
- `POST /api/bookings` отвечает `403` — бронь нельзя создать в обход интерфейса;
- уже созданные брони сохраняются, админские команды бота работают как обычно.

Состояние хранится в таблице `settings` (ключ `season_closed`) и переживает перезапуск сервера.
